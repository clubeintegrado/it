...
object AlertDialogBuilder {
    ...
    fun show(context: Context, message: String?, title: String? = context.getString(R.string.message_default_title), button: DialogButton = DialogButton.OK, event: AlertDialogEvent?) {
        ...
        if (button == DialogButton.OK || button == DialogButton.OK_CANCEL) {
            ...
            materialDialogBuilder.setPositiveButton(positiveText) { dialog: DialogInterface?, which: Int -> event?.let { it.onAnswerDialog(DialogResult.OK) } }
        }
        ...
        if (button == DialogButton.CANCEL_RETRY) {
            ...
            materialDialogBuilder.setPositiveButton(positiveText) { dialog: DialogInterface?, which: Int -> event?.let { it.onAnswerDialog(DialogResult.CANCEL) } }
        }
        materialDialogBuilder.setOnCancelListener { dialogInterface: DialogInterface? -> event?.let { it.onAnswerDialog (DialogResult.CANCEL) } }
