...
class CouponsCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    @IBAction func onAddCouponClicked(_ sender: UIBarButtonItem) {
        let title = NSLocalizedString("coupon.message.add.content",value: "Add Coupon", comment: "Add Coupon message title")
        let message = NSLocalizedString("coupon.message.add.content", value: "Enter your coupon code",comment: "")
        let dialog = UIAlertController(title: title, message: message, preferredStyle: .alert)
        dialog.addTextField() { textField in
            textField.placeholder = NSLocalizedString("coupon.message.add.placeholder", value: "Coupon Code",comment: "")
        }
        ...
        dialog.addAction(UIAlertAction(title: NSLocalizedString("message.button.cancel",value: "Cancel", comment: "Message Cancel Button"), style: .cancel))
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if selectMode {
            Coupon.shared = coupons[indexPath.row]
            if delegate != nil {
