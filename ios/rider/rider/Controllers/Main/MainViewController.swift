...
class MainViewController: UIViewController, CLLocationManagerDelegate, ServiceRequested {
    ...
    @IBAction func onMenuClicked(_ sender: UIBarButtonItem) {
        ...
        if(!imageMarker.isHidden) {
            ...
            imageMarker.image = (pointsAnnotations.count == 0 ? UIImage(named: "marker_pickup") : UIImage(named: "marker_destination"))
            return
        }
    ...
    @IBAction func onFavoritesClicked(_ sender: UIBarButtonItem) {
        GetAddresses().execute() { result in
            ...
            case .success(let response):
                ...
                let dlg = UIAlertController(title: NSLocalizedString("favorites.picker.title",value: "Favorite Addresses", comment: "Favorites Picker Title"), message: NSLocalizedString("favorites.picker.description",value: "Chose the location from below picker", comment: "Favorites Picker Description"), preferredStyle: .alert)
