...
class DriverTravelViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    @IBOutlet weak var labelDistanceTraveled: UILabel!
        ...
        Finish(confirmationCode: finishService.confirmationCode, distance: finishService.distance ?? 0, log: finishService.log ?? "").execute() { result in
            ...
            case .success(let response):
                ...
                if response.status {
                    ...
                    DialogBuilder.alertOnError(message: "Waiting for user payment.")
                }
    ...
    private func refreshScreen(travel: Request = Request.shared) {
        ...
        case .Started:
            ...
            for point in travel.points.dropFirst() {
                let ann = MKPointAnnotation()
        ...
        case .WaitingForPostPay:
            DialogBuilder.alertOnError(message: "Waiting for payment")
            LoadingOverlay.shared.showOverlay(view: self.view)
    ...
    func openNavigationMenu(location: CLLocationCoordinate2D) {
        ...
        alert.addAction(UIAlertAction(title: "Yandex.Maps", style: .default, handler: { action in
            if let url = URL(string: "yandexmaps://maps.yandex.com/?ll=\(location.longitude),\(location.latitude)&z=14"),
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        labelDistanceTraveled.text = String(format:NSLocalizedString("base.distance.km", value: "%.1f km", comment: "Default format for distances in km"), Double(exactly: Request.shared.distanceReal!)! / Double(exactly: 1000)!)
